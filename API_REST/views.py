...
class Estudiantes(APIView):
    ...
    def get(self, request):
        ...
        if(id_grupo == 0):
            ...
            listaEstudiantes = Estudiante.objects.values('id', 'nombres', 'apellidos','grupoxestudiante__fila', 'grupoxestudiante__columna', \
            'sexo_biologico', 'descripcion').filter(grupoxestudiante__grupo=id_grupo)
...
class SeguimientoXEstudiante(APIView):
    def post (self, request):
        try:
            ...
            categorias = Categoria.objects.values('id').exclude(id__in=(padres)).filter(habilitada=True)
...
class EstadisticaGrupal2(APIView):
